...
def joinCitiesData():
    ...
    cur.execute("SELECT CitiesData.Quality_of_Life, CitiesData.Average_Annual_Salary, CitiesData.Population FROM CitiesData JOIN RestaurantCities ON trim(CitiesData.CityID) = trim(RestaurantCities.ID)")
    conn.commit()
    joined = cur.fetchall()
    for x in joined: 
        print(x)
